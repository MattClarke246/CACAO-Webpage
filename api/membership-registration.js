...
module.exports = async function handler(req, res) {
  ...
  const membershipPlan = stripHtml(String(body.membershipPlan || "").trim()).slice(0, 100);
  ...
  try {
    await resend.emails.send({
      ...
      to: toEmail,
      replyTo: email,
      subject: `[CACAO Membership] New Application — ${firstName} ${lastName}`,
      text: lines.join("\n"),
    });
    ...
    return res.status(500).json({ error: "Unable to send application at the moment." });
  }
};
